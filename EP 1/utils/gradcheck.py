    ...
    while not it.finished:
        ...
        if reldiff > 1e-5:
            ...
            print(
                ...
                % str(ix))
            print("Seu gradiente: %f \t gradiente numérico: %f" %
                  (grad[ix], numgrad))
            return
...
def grad_tests_softmax(skipgram, dummy_tokens, dummy_vectors, dataset):
    print(
        "======Casos de teste para Skip-Gram com naiveSoftmaxLossAndGradient ======"
    )
    ...
    expected_gradCenterVecs = [[0., 0., 0.], [0., 0., 0.],
...
def grad_tests_negsamp(skipgram, dummy_tokens, dummy_vectors, dataset,
